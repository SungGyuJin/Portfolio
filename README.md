...
>(추후 계속 업데이트 예정)
> * 05/16(금) Update
> > &nbsp;&nbsp;&nbsp;&nbsp;작업도중 메인화면에 한개가 아닌 여러개의 포트폴리오들을 Card 형태의 목록으로 나열해보면 어떨까? 라는 생각을 했습니다. 물론 지금은 한 개뿐이라 메인에는 Card 하나만 나오지만 또 다른종류의 포트폴리오가 생기면 2개가 되고, 3개가 되는 형식으로 방향을 잡았습니다. 그래서 메인화면 제목을 "Portfolio Collection" 이라고 정했습니다. 화면을 왔다 갔다 하는 방식보다 원하는 포트폴리오를 클릭하면 Modal을 띄워서 그 안에서 동작시켜보는, 그리고 그 포트폴리오는 관리자 페이지와 연결되어 있는 형식으로 작업했습니다. 그리고 현재는 메인화면 배너이미지, 문구, nav바 등 데이터가 고정된 형태지만 이 모든 것을 관리자 화면에서 다 조절할 수 있도록 수정할 예정입니다.
