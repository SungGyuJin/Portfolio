...
> 삭제 역시 가능합니다. 삭제를 하면 메인화면에서는 보여지지 않으나, 관리자 화면에서는 삭제된 데이터로 남게됩니다. 영구삭제까지 해야만 관리자 화면에서도 보이지 않게 됩니다. (수정시 수정일시만 변경됨)

### 3) Portfolio
![5  Portfolio](https://github.com/user-attachments/assets/e41d9836-be0b-41fb-ba0d-aec0c306f03d)
> 기술 영역과 같은 방식입니다. 삭제의 경우 역시 메인화면에서는 보여지지 않으나, 관리자 화면에서는 삭제된 데이터로 남게되며 영구삭제까지 해야만 관리자 화면에서도 보이지 않게 됩니다. (수정시 수정일시만 변경됨)
...
# 3. 마이페이지
> 마이페이지 기능입니다. 로그인을 하고 게시판 메인화면 진입시 좌측 상단에서 자신이 쓴 게시글과 댓글, 임시 저장한 글 들을 확인 할 수 있습니다. 그리고 프로필 클릭시 이름, 비밀번호, 프로필 이미지를 수정할 수 있고, 계정 탈퇴 신청도 가능합니다.

### 1) 정보 수정
![1  정보 수정](https://github.com/user-attachments/assets/2b0ecab3-2d06-4675-a9af-857a53c94fb0)
> 정보 수정은 비밀번호와 이름 변경, 그리고 프로필 이미지 등록, 수정, 삭제가 가능합니다. 비밀번호 변경은 하지 않고 이름이나 프로필 이미지만 수정하고 싶을때는 비밀번호 입력란은 아무거도 입력하지 않고 진행하면 되도록 작업했습니다.
> 만약 비밀번호 변경을 원한다면 비민번호 항목과 비밀번호 확인 항목 두개를 꼭 똑같이 입력해야만 수정이 가능하도록 했습니다. ("변경을 원할시 입력" 이라는 문구추가)
>
> 저장버튼 옆의 새로고침 버튼은 가장 원래 정보상태로 돌아오는 역할을 하며, 게시물의 작성자 공개는 첫 글자와 마지막 글자 외에는 *표로 마스킹 처리했습니다.

### 2) 계정 탈퇴
![2  계정 탈퇴](https://github.com/user-attachments/assets/43648acd-64e6-41bb-98e3-3ba72a118609)
> 계정 탈퇴 과정을 담은 모습입니다.
> 1. 사용자의 탈퇴신청
> 2. 관리자의 탈퇴처리
> 
> 크게 보면 위의 과정이 전부입니다만 세부적으로 들어가면 아래와 같습니다.
>
> 사용자가 탈퇴를 원할시 탈퇴 신청버튼을 클릭 후 계정 비밀번호를 다시 한번 입력합니다. 그 후 회원 관리 목록(탈퇴)으로 정보가 옮겨진 것을 확인할 수 있습니다. 이후 관리자는 복구, 탈퇴처리 중 하나를 선택해 수행할 수 있습니다.
> 
> 탈퇴 처리를 하게되면 보시다시피 사용자가 로그인 했을때 탈퇴 처리되었다는 문구와 함께 이제는 더 이상 로그인을 할 수 없습니다.
> 관리자가 복구 처리를 해주어야만 사용자가 로그인 할 수 있도록 작업했습니다. 
