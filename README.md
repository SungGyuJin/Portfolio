...
#### 나의 생각 및 전체적인 개선사항 정리
>> * 05/16(금) update
>> 현재까지 느낀점을 적어보자면 작업을 할수록 예외처리도 너무 많고, 변수도 정말 많다는 것을 여전히 느낀다. 뭐 예상은 한거지만... 이런게 난 재미라고 생각한다. 영상을 따로 첨부하진 않았지만 예를 들어 처음에 로그인, 회원가입 기능을 만들고 사용자 화면까지 다 만들어서 nav바에 LOGIN이라는 메뉴를 넣고 보니 "어.. 이전에 만든 로그인 페이지는 관리자를 위한 로그인 페이지인데? 그런데 회원가입에 보면 관리자 게시물 데이터들을 날짜별, 게시판별로 Excel로 내려받기가 가능하게끔 기능추가
- 공지사항의 경우 사용자 화면에서 제일 상단에 노출되도록 작업예정
...
> - 첫번째 영상은 사용자 화면의 비밀글입니다. 제목 앞부분에 자물쇠 아이콘을 추가하였고, 게시글 제목 클릭시 비밀번호 입력 창이 나타나도록 했습니다.
> - 두번째 영상은 관리자 화면의 비밀글입니다. 제목 앞부분 자물쇠 아이콘까지는 사용자 화면과 같지만 관리자 페이지 특성상 비밀번호를 입력하지 않도록 작업했습니다. 
